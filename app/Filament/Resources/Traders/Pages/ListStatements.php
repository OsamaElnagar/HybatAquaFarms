...
class ListStatements extends Page implements HasTable
{
    ...
    protected string $view = 'filament.resources.traders.pages.list-statements';
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                TraderStatement::query()
                    ->where('trader_id', $this->record->id)
                    ->latest('opened_at')
            )
            ->columns([
                ...
                TextColumn::make('closing_balance')
                    ...
                    ->money('EGP', locale: 'en')
                    ->placeholder('—'),
                TextColumn::make('harvestOperations.operation_number')
                    ->label('عمليات الحصاد')
                    ->badge()
                    ->color('info'),
            ])
            ->actions([
                Action::make('viewStatement')
                    ->label('عرض الكشف')
                    ->icon('heroicon-o-eye')
                    ->url(fn (TraderStatement $record) => TraderResource::getUrl('statement', [
