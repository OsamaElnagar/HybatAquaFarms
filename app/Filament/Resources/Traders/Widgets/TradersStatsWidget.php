...
class TradersStatsWidget extends StatsOverviewWidget
{
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('المبيعات الإجمالية', number_format($totalRevenue, 2).' ج.م')
                ->description('إجمالي قيمة أوامر البيع')
                ->descriptionIcon('heroicon-o-shopping-cart')
                ->color('success'),

            Stat::make('المستحقات', number_format($totalReceivables, 2).' ج.م')
                ->description($pendingOrders.' أمر بيع معلق')
                ->descriptionIcon('heroicon-o-banknotes')
                ->color($totalReceivables > 0 ? 'warning' : 'success'),

            Stat::make('متوسط قيمة الطلب', $totalTraders > 0 ? number_format($totalRevenue / max(1, SalesOrder::count()), 2).' ج.م' : '0.00 ج.م')
