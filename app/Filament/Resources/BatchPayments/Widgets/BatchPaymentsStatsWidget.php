...
class BatchPaymentsStatsWidget extends StatsOverviewWidget
{
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('إجمالي المدفوعات', number_format($total, 2).' ج.م')
                ->description('عدد العمليات: '.number_format($count))
                ->descriptionIcon('heroicon-o-banknotes')
                ->color('primary'),

            Stat::make('متوسط الدفع', number_format($avg, 2).' ج.م')
                ->description('لكل عملية')
                ->descriptionIcon('heroicon-o-adjustments-vertical')
                ->color('info'),

            Stat::make('مدفوعات هذا الشهر', number_format($thisMonth, 2).' ج.م')
